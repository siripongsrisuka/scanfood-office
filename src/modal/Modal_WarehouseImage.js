...
function Modal_WarehouseImage({
  ...
  setCurrent,
    ...
    const { imageUrls = [], comment = '' } = current;
    ...
    >
      ...
      <Modal.Body  >
        <InputArea
            name='comment'
            placeholder="คำแนะนำ"
            onChange={(event)=>{setCurrent({...current,comment:event.target.value})}}
            value={comment}
            strict={true}
            rows={8}
        />
...
export default Modal_WarehouseImage;
