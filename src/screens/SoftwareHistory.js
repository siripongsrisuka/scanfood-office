...
function SoftwareHistory() {
    ...
    function handleOwner({ id:ownerId }){
        ...
        setLoading(true)
        try {
            db.collection('packageOrder').doc(id).update({ ownerId, ownerName })
    ...
    const exportToXlsx = () => {
        const data = masterData.map((a) => {
          const { 
            orderNumber, timestamp, net, shopName, adminName, packageName, ownerId, ownerName
          } = a;
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, `ผลงาน Software ของ ${IsName[filter]}`);
      
        const fileName = `ผลงาน Software ของ ${IsName[filter]} ${stringYMDHMS3(startDate)}-${stringYMDHMS3(endDate)}.xlsx`;
