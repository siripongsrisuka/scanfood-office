...
import { Modal_Equipment, Modal_FlatListTwoColumn, Modal_Loading, Modal_OneInput } from "../modal";
...
function WarehouseJobScreen() {
    ...
    const options = useMemo(()=>{
        return [
                ...
                { id:'3', name:"ส่งแล้ว", value:masterData.filter(a=>a.status==='sent').length  },
                { id:'4', name:"ทั้งหมด", value:masterData.length  },
            ]
    },[masterData])
    ...
    useEffect(()=>{
        let arr = optionId === '4'
            ...
            :masterData.filter(a=>{
                ...
                if(optionId === '3') return a.status==='sent'
            });
    ...
    async function fetchJobs(){
        ...
        try {
            const query = await db.collection('hardwareOrder')
                .where('status','not-in',['success','cancel'])
                .get();
